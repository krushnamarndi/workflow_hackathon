...
export default function FolderPage() {
  ...
  // Fetch folder data
  const { data: folderData, isLoading, refetch } = trpc.folder.get.useQuery({ id: folderId });
  const { data: breadcrumb } = trpc.folder.getBreadcrumb.useQuery({ id: folderId });

  // Search query
  const { data: searchResults } = trpc.folder.search.useQuery(
    { query: searchQuery, folderId },
  ...
  // Normalize the data structure - search results use 'folders', folder data uses 'children'
  const displayItems = searchQuery.length > 0 
    ? searchResults 
    : folderData 
      ? { folders: folderData.children, workflows: folderData.workflows }
      : null;
    ...
    <div className="flex h-screen bg-[#0A0A0A] text-white font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="flex items-center justify-between p-4 md:p-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-medium text-zinc-300">
              {isLoading ? 'Loading...' : folderData?.name || 'Folder'}
            </h1>
          </div>
          <button 
            onClick={handleCreateFile}
        ...
        <div className="px-4 md:px-8 flex flex-col gap-6 md:gap-8 pb-12">
          {/* Top Selection - Workflow Library */}
          ...
          <section>
            ...
            <div className="flex items-center justify-between mb-4 md:mb-6">
              <h2 className="text-base md:text-lg font-medium">Files</h2>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 md:gap-6">
                {displayItems?.folders?.map((folder: any) => (
                  <FileCard
                    ...
                    onMove={() => setMoveDialog({ open: true, id: folder.id, name: folder.name, type: 'folder', folderId: folder.parentId, folderName: folderData?.name })}
                  />
                ))}
                {displayItems?.workflows?.map((workflow: any) => (
                  <FileCard
                    ...
                    onMove={() => setMoveDialog({ open: true, id: workflow.id, name: workflow.name, type: 'workflow', folderId: workflow.folderId, folderName: folderData?.name })}
                  />
                ))}
